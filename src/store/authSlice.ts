...
const slice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(registerUser.fulfilled, (s, a) => { s.loading = false; s.user = a.payload; localStorage.setItem('mt_current_user', JSON.stringify(a.payload)) })
      .addCase(registerUser.rejected, (s, a) => { s.loading = false; s.error = a.payload as string })
      .addCase(loginUser.pending, (s) => { s.loading = true; s.error = undefined })
      .addCase(loginUser.fulfilled, (s, a) => { s.loading = false; s.user = a.payload; localStorage.setItem('mt_current_user', JSON.stringify(a.payload)) })
